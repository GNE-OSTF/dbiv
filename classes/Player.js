...
function Player(name, playerNr)
{
  ...
  var dept_ = 0;
  var field_ = 1;
  var playerNr_ = playerNr; // is unique for room
  //var ready_ = false;
  var ownedFields = [];
  var state_ = 'notReady';
  ...
  this.getDept = function() {return dept_;}
  this.getField = function() {return field_;}
  this.getPlayerNr = function() {return playerNr_;}
  this.isReady = function() {return (state_ != 'notReady');}
  ...
  this.setState = function(state) {state_ = state;}
  ...
  this.getPlayerInfo = function() {
    ...
    if(charID_ === '') {
      ...
      info['charID'] = charID_ + 1;
      info['charName'] = charName_;
      info['status'] = this.getStatus();
      info['income'] = this.getSalary();
      info['moneysack'] = money_;
      info['dept'] = dept_;
      info['inPrison'] = in_prison_;
    }
  ...
  this.getStatusInfo = function() {
    ...
    info['majority'] = this.getMajority();
    info['age'] = this.getAge();
    info['education'] = this.getEducationLevel();
    
    info['language'] = this.getLanguage();
    info['HrTrainer'] = this.getHrTrainer();
    info['Licence'] = this.getLicence();
    info['It'] = this.getIt();
    info['Mba'] = this.getMba();
    
    info['EuCitizen'] = this.getEuCitizen();
    info['Nostrification'] = this.getNostrification();
    info['Residency'] = this.getResidency();
    
    info['Car'] = this.getCar();
    info['FreeLawyers'] = this.getFreeLawyers();
    info['FreeCarRepairs'] = this.getFreeCarRepairs();
    info['FreeJailbreaks'] = this.getFreeJailbreaks();
    
    return info;
  }

  this.getSpecificPlayerInfo = function() {
    var info = {};
    
    info['playerInfo'] = this.getPlayerInfo();
    info['statusInfo'] = this.getStatusInfo();
    info['state'] = state_;
